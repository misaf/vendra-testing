...
- `makeCurrentTestTenantWithFeatures()` establishes deterministic Pennant state, and `setUpFilamentSuperAdminTestContext()` boots a configured admin panel, user, role, tenant, resources, and feature set without importing a concrete permission package.
- `toSortByEverySortableColumn()` verifies every sortable Livewire table column in ascending order and, by default, descending order.
- **Module test suites must use these helpers instead of importing `Misaf\VendraTenant`.** Only `vendra-tenant` tests may import the concrete provider; the root `tests/Feature/PackageManifestConsistencyTest.php` guard enforces this.
